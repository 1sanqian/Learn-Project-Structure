...
异步是在遇到特定的异步修饰符时，不会等待方法完成，而是直接进入下一个方法或程序中，可在异步操作执行时继续执行，执行方法是无序的，且执行的速度也比较快。async await 操作，await 会标记异步方法在暂停点，但主线程执行结束后，会不断的去询问堆栈中是否有等待需要执行的异步方法，如果有则执行，然后把暂停点的值返回给主线程。
